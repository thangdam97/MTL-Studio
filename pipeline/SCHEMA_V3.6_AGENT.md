...
```json
{
  ...
  "metadata": {
    ...
    "translation_guidance": {
      "genres": ["romcom" | "ecchi" | "slice_of_life" | "action" | "drama" | "fantasy"],
      "translator_notes": {
      ...
      "grammar_rag_integration": {
        "enabled": boolean,
        "pattern_priorities": {"pattern_name": number}
      }
    }
  },
  "structure": {
...
12. **Generate genre-specific translator notes**:
    ```
    Based on genre analysis, populate translation_guidance:
    
    For romcom volumes:
    - priority_patterns: ["comedic_timing", "tsukkomi_interjection", "escalation_repetition"]
    - tone: "Punchy, blunt punchlines. Keep absurdity matter-of-fact. Preserve repetition for comedic effect."
    - avoid: ["Over-embellishing comedy", "smoothing out awkward repetition", "softening insults"]
...
### Phase 7: Schema v3.6 Upgrade

14. **Run the automated schema upgrade script**:
    ```bash
...
15. **Complete the character profiles**:
...
16. **Fill content_info fields**:
    ...
    "translation_guidance": {
      "genres": ["romcom", "ecchi"],
      "translator_notes": {
        "priority_patterns": ["comedic_timing", "matter_of_fact_absurdity", "blunt_descriptor"],
        "tone": "Punchy, blunt punchlines. Direct, no euphemisms for ecchi situations.",
        "avoid": ["Over-embellishing comedy", "purple prose", "softening insults"]
      },
      "grammar_rag_integration": {
        "enabled": true,
        "pattern_priorities": {
          "comedic_timing": 1.5,
          "matter_of_fact_absurdity": 1.3,
          "tsukkomi_interjection": 1.4
        }
      }
    }
    ```
    
17. **Verify all placeholders are replaced**:
