...
if not VOLUME_DIR:
    print("❌ Error: Could not find 05df volume directory")
    exit(1)
...
# Output paths
EN_OUTPUT = VOLUME_DIR / "05df_EN_Full.md"
JP_OUTPUT = VOLUME_DIR / "05df_JP_Full.md"
...
def merge_english():
    ...
    output_content.append(f"**Translation Model:** Gemini 2.0 Flash Thinking Experimental\n\n")
    output_content.append(f"**Quality Grade:** A (93.4/100) - Professional Publication Standard\n\n")
    output_content.append(f"**Volume ID:** {manifest['volume_id']}\n\n")
    
    # Add synopsis if available
    if 'synopsis' in manifest['metadata_en']:
...
def main():
    print("=" * 80)
    print("MTL Studio - Volume 05df Merger")
    print("Volume: 人生に疲れ辿り着いた村で、美女達が俺を離してくれない")
    print("=" * 80)
    ...
    print(f"\n📘 Volume Status: Phase 3 Complete - Grade A (93.4/100)")
    print(f"   Content Fidelity: 99.71% (0.29% deviation)")
    print(f"   Ready for Phase 4 (EPUB Build)")
